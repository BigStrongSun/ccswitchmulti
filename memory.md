...
- `v3.19.2-30@9dcb3bc5` 推送后，GitHub CI run `34021819218` 在 macOS 严格 Clippy 暴露 `codex_paginated_history_repair.rs` 的条件编译边界：活动 lineage 重构仍在所有平台执行投影游标安全检查，但检查结果只在 Windows/测试目标写入候选结构，非 Windows 目标没有显式消费局部变量。Release run `34021819064` 在公开发布前已取消；不可变 v30 tag 保留且不移动、不复用，正式版本顺延为 v3.19.2-31。根修显式表达非 Windows 的只读验证消费语义，不使用全局 `allow` 掩盖诊断。本地分页历史聚焦回归 10/10、rustfmt、`cargo check --all-targets` 与严格 Clippy 均通过；必须再让包含修复的精确 main commit 通过三平台 CI，才允许创建 v31 tag。
