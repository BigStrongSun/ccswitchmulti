...
- 32 页均在标题后加入独立的“这页用大白话说”说明框，用一至两句中文交代“这页讲什么、为什么重要”；专业字段、函数名和协议名继续保留英文，以便读者对应 Codex/OpenAI/CCSM 源码。32 条说明必须与页面一一对应、内容互不重复，运行时数量不一致或页面缺标题时直接抛错，不能静默漏页。
- 面向中文初学者时，优先采用 OpenAI 官方中文语境中稳定的“智能体、多智能体工作流、独立环境、任务委托”等说法；必须保留的英文术语旁边补直白中文解释，不用中英文混排本身代替解释。
- 应用内浏览器 1280×720 对 32 页逐页核对：每页 `scrollWidth == clientWidth`、`scrollHeight == clientHeight`，无元素越界和 console error。
- 加入大白话层后，默认桌面视口 32/32 页均有说明框且无横向溢出；仅第 3、4、7 页需要少量纵向滚动。900×650 断点下再次逐页验收，32/32 页无横向溢出、所有长页 `overflow:auto` 可滚动，代码页保持至少 12.5px，表格页和关键时序页未出现横向裁切。
- html-ppt runtime 翻页有约 500ms 过渡。截图若只等待 120ms，会把上一页过渡残影误判为静态叠层；视觉验收应等待至少 900ms，并确认只有一个 `.slide` 的 computed opacity 大于 0，再截图。
- 最终截图覆盖封面、V1 结构、V2 六层结构、第三方四层障碍、CCSM 数据面、新 Role 流水线和踩坑矩阵；加入大白话层后必须重新生成，不能沿用旧截图。
- 网格穿透卡片并非网格层级跑到正文上方，而是 `style.css` 使用了三套主题都未定义的 `--surface-1`，使整个 `background` 声明失效；同时部分规则显式混入 `transparent`。正文承载表面统一改用主题已有且不透明的 `--surface`，网格只留在页面空白区。必须同时验收卡片、时序事件框、大白话框、结论框、状态/漏斗节点和 exploration 入口，并覆盖 engineering-whiteprint、academic-paper、tokyo-night 三套主题与 900×650 断点。
