...
- Chat Completions 分支通过 `ChatToolAccumulator` 累加 delta 后再判定，不具有这条“首个 added 事件抢跑”的同型缺陷。修复应在 Responses 提取层优先采用终态证据（`response.output_item.done` 或 `response.completed.response.output`），并仅接受 completed 且参数完整的 function call；必须用 DeepSeek 的 `added(empty) -> argument deltas -> done(complete) -> completed` 真实序列先建立 RED 回归，再改生产代码。本次仅完成只读诊断、官方文档交叉验证和 memory 记录，未修改 runner、未重启或替换安装版。
