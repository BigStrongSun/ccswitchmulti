...
- 实现分支为 `bigstrongsun/probe-production-request-equivalence`，从 `07f4adb0` 开始；核心提交依次为 `31d5440e`（边界设计）、`7e7830ea`（共享请求契约 RED）、`60594a38`（生产 forwarder 共享请求策略）、`0f021433`（candidate/production 等价 RED）、`40d72d88`（probe 复用生产请求策略）、`3b2ca15a`（probe 复用生产终态分类）、`5a126f6e`（迁移旧运行时档案夹具）和 `59fa6210`（测试辅助方法仅在测试构建可见）。本分支未推送、未合并、未安装、未发布。
