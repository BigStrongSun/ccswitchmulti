...
/// Whether this MultiRouter must emit plaintext V2 agent tasks.
///
/// A child may use any enabled route. If one route is third-party or its credential
/// ownership is ambiguous, OpenAI-only encrypted task arguments are not portable.
/// Resolved providers retain the request-local marker so retry-layer materialization
/// cannot erase this decision.
