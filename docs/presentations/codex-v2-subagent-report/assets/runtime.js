...
(function () {
  ...
  ready(function () {
    ...
    function go(n, fromRemote){
      ...
      // hash
      const exploreHash = /^#\/(\d+)\/explore\/([a-z0-9-]+)$/.exec(location.hash || '');
      const preserveExplore = exploreHash && Number(exploreHash[1]) === n + 1;
      const hashTarget = preserveExplore ? location.hash : '#/'+(n+1);
      if (location.hash !== hashTarget && !isPresenterWindow) {
        history.replaceState(null,'', hashTarget);
      }

      document.dispatchEvent(new CustomEvent('htmlppt:slide-change', {
        detail: { index: n, slide: slides[n] }
      }));
