...
describe("PPIO provider presets", () => {
  ...
  it("configures Claude Code discovery against the OpenAI models endpoint", () => {
    expect(getPpioPreset(providerPresets)).toMatchObject({
      ...ppioBrandFields,
      settingsConfig: {
        env: {
  ...
  it("configures OpenAI-compatible clients with the versioned base", () => {
    expect(getPpioPreset(opencodeProviderPresets)).toMatchObject({
      ...ppioBrandFields,
      settingsConfig: {
        npm: "@ai-sdk/openai-compatible",
        name: "PPIO",
        options: { baseURL: ppioOpenAiEndpoint, apiKey: "", setCacheKey: true },
        models: { [ppioModelId]: { name: ppioModelName } },
      },
    });
    expect(getPpioPreset(openclawProviderPresets)).toMatchObject({
      ...ppioBrandFields,
      settingsConfig: {
    ...
    expect(getPpioPreset(hermesProviderPresets)).toMatchObject({
      ...ppioBrandFields,
      settingsConfig: {
        name: "ppio",
        ...
        models: [
          { id: ppioModelId, name: ppioModelName, context_length: 1048576 },
        ],
      },
      suggestedDefaults: { model: { default: ppioModelId, provider: "ppio" } },
    });
  });

  it("configures Pi with the DeepSeek V4 thinking contract", () => {
    expect(getPpioPreset(piProviderPresets)).toMatchObject({
      ...ppioBrandFields,
